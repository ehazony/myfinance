...
def expenses_transactions(user):
	...
	return Transaction.objects.filter(date__gte=start_date, date__lte=end, user=user).exclude(tag__name__in=['Exclude', 'Credit Cards', 'Salary'])
...
# 	fig = px.line(trans, x="month_date", y="value__sum", color="tag__name", line_group="tag__name",
# 	              hover_name="tag__name")
# 	return fig
...
# 	trans = trans.values('tag__name').annotate(Sum('value'))
# 	if trans.count() == 0:
# 		return None
# 	trans = pd.DataFrame(list(trans))
#
...
# 	trans = trans.rename(columns={'tag__name': 'tag', 'value__sum': 'monthly average'}, index={'ONE': 'one'})
