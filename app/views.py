...
class MonthTrackingView(APIView):
    ...
    def get(self, request, format=None):
        ...
        transactions = Transaction.objects.exclude(
            tag_ref__name__in=['credit cards', 'bills', 'salary', 'same', 'debt payment', 'Donations', 'other income',
                               'commission', 'exclude', 'vacation']).filter(date__gte=start_month, date__lte=end_month,
                                                                            user=request.user)
        tag_sums = transactions.values(tag_id=F('tag_ref')).annotate(Sum('value'))
        total = 0
        ...
        for tag_sum in tag_sums:
            ...
            t = '\n' + '{}: {}/ {}'.format(tag.name.replace('_', ' ').capitalize(),
                                           value_sum, str(int(tag.taggoal_set.first().value)))
        ...
        tag_goals = tag_goals.exclude(tag__id__in=[tag_sum['tag_id'] for tag_sum in tag_sums])
        ...
        asyncio.run(telegram_bot_api.send_message(s))
...
class MonthCategoryView(APIView):
    ...
    def get(self, request, format=None):
        ...
        end_month = datetime.datetime.now().replace(day=calendar.monthrange(start_month.year, start_month.month)[1],
                                                    minute=0, second=0, microsecond=0)
        transactions = Transaction.objects.exclude(
            tag_ref__name__in=['credit cards', 'bills', 'salary', 'same', 'debt payment', 'Donations',
                               'other income',
