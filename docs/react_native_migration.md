...
1. **Project Setup**
   - [x] Initialize Git repository for the new app (in a separate directory)
   - [x] Create `README.md` with setup instructions for Expo and yarn
   - [x] Ensure `api/openapi.yaml` is committed from the backend and runnable by the `generate:api` script.

2. **Dependencies**
   - [x] Install React Navigation, Redux Toolkit, React Query, React Native Paper, React Hook Form, Yup, and testing libraries
   - [ ] Install msw-openapi (NOT INSTALLED: not available on npm, see chat log for details)
   - [ ] Install @testing-library/react-hooks (NOT INSTALLED: incompatible with React 18, see chat log for details)
   - [x] Configure TypeScript path aliases in `tsconfig.json`
   - [x] Install Expo Router, FlashList, NativeWind (and/or Tamagui), Reanimated 3, Moti, Zod, Sentry, Detox, Storybook 7, openapi‑zod‑client
